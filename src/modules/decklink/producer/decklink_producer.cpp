...
struct Filter
{
           ...
           bool                                 hdr)
    {
        ...
        if (type == AVMEDIA_TYPE_VIDEO) {
            ...
            AVPixelFormat pix_fmts[] = {pix_fmt};
            FF(av_opt_set_array(
                sink, "pixel_formats", AV_OPT_SEARCH_CHILDREN, 0, 1, AV_OPT_TYPE_PIXEL_FMT, pix_fmts));
#else
...
#if LIBAVFILTER_VERSION_INT >= AV_VERSION_INT(10, 6, 100)
            AVSampleFormat sample_fmts[]  = {AV_SAMPLE_FMT_S32};
            int            sample_rates[] = {format_desc.audio_sample_rate};

            FF(av_opt_set_array(
                sink, "sample_formats", AV_OPT_SEARCH_CHILDREN, 0, 1, AV_OPT_TYPE_SAMPLE_FMT, sample_fmts));
            FF(av_opt_set_array(
                sink, "samplerates", AV_OPT_SEARCH_CHILDREN, 0, 1, AV_OPT_TYPE_INT, sample_rates));
#else
