...
namespace caspar
{
    namespace aja
    {
        ...
        namespace
        {
            ...
            ULWord successful_field_transfers_ = 0;

            inline std::uint8_t clamp_byte(int value)
            {
                return static_cast<std::uint8_t>(
                    std::max(0, std::min(255, value)));
            }

            void bgra_to_uyvy(const std::uint8_t *src,
                              std::uint8_t *dst,
                              std::size_t width,
                              std::size_t height)
            {
                ...
                int first_line)
            {
                     ...
                     y += 2, ++out_y)
                {
                    ...
                    bgra_to_uyvy(
                        src + y * src_stride,
                        dst + out_y * dst_stride,
                        width,
                        1);
                }
            }

            void bgra_field_to_interlaced_uyvy(
                const uint8_t *src,
                uint8_t *dst,
                int width,
                int height,
            ...
            public:
                                ...
                                int port_index) override
                {
                    ...
                    CASPAR_LOG(info)
                        << L"AJA consumer initializing for Caspar channel "
                        << channel_index_;

                    if (format_desc.width != 1920 ||
                        format_desc.height != 1080 ||
                        format_desc.format != core::video_format::x1080i5000)
                    {
                        CASPAR_THROW_EXCEPTION(
                            user_error()
                            << msg_info("Initial AJA consumer supports only 1080i5000"));
                    }
                    ...
                    if (!CNTV2DeviceScanner::GetDeviceAtIndex(kDeviceIndex, device_))
                    {
                        CASPAR_THROW_EXCEPTION(
                            user_error()
                            << msg_info("Unable to open AJA device 0"));
                    }
                    ...
                    if (!device_.EnableChannel(kChannel))
                    {
                        CASPAR_THROW_EXCEPTION(
                            caspar_exception()
                            << msg_info("Unable to enable AJA channel 1"));
                    }
                    ...
                    if (!device_.SetVideoFormat(kVideoFormat, false, false, kChannel))
                    {
                        CASPAR_THROW_EXCEPTION(
                            caspar_exception()
                            << msg_info("Unable to set AJA video format to 1080i50"));
                    }
                    ...
                    const NTV2Standard video_std =
                        GetNTV2StandardFromVideoFormat(kVideoFormat);

                    device_.SetSDIOutputStandard(
                        kChannel,
                        video_std);

                    device_.SetSDIOutLevelAtoLevelBConversion(
                        kChannel,
                        false);

                    device_.SetSDIOutRGBLevelAConversion(
                        kChannel,
                        false);
                    ...
                    const NTV2OutputXptID source_xpt =
                    ...
                    if (!device_.ApplySignalRoute(connections, true))
                    {
                        CASPAR_THROW_EXCEPTION(
                            caspar_exception()
                            << msg_info("Unable to apply AJA SDI routing"));
                    }
                            ...
                            AUTOCIRCULATE_WITH_RP188))
                    {
                        CASPAR_THROW_EXCEPTION(
                            caspar_exception()
                            << msg_info("Unable to initialize AJA AutoCirculate output"));
                    }
                    ...
                    CASPAR_LOG(info)
                        << L"AJA consumer initialized: device 0, channel 1, "
                        << L"1080i50, 8-bit YCbCr, video only";
                }
                                       ...
                                       core::const_frame frame) override
                {
                    ...
                    try
                    {
                        ...
                        // CASPAR_LOG(info)
                        //     << L"AJA send: field=" << static_cast<int>(field)
                        //     << L", format=" << static_cast<int>(desc.format)
                        //     << L", size=" << frame.size()
                        //     << L", width=" << frame.width()
                        ...
                        {
                            CASPAR_LOG(error)
                                << L"AJA consumer received unsupported pixel format "
                        ...
                        const std::size_t expected =
                            static_cast<std::size_t>(1920) *
                            static_cast<std::size_t>(1080) * 4u;

                        if (image.size() < expected)
                        {
                            CASPAR_LOG(error)
                                << L"AJA consumer received undersized BGRA frame: "
                                << image.size()
                                << L" bytes, expected at least "
                                << expected;
                        ...
                        NTV2FieldID output_field_id = NTV2_FIELD_INVALID;

                        if (device_.GetOutputFieldID(kChannel, output_field_id))
                        {
                            CASPAR_LOG(info)
                                << L"AJA send: Caspar field "
                                << (field == core::video_field::a ? L"A" : L"B")
                                << L", output field ID "
                                << static_cast<int>(output_field_id);
                        }

                        AUTOCIRCULATE_TRANSFER transfer;

                        transfer.SetVideoBuffer(
                            reinterpret_cast<ULWord *>(video_buffer_.data()),
                            static_cast<ULWord>(video_buffer_.size()));

                        if (!device_.AutoCirculateTransfer(kChannel, transfer))
                        {
                            CASPAR_LOG(error)
                                << L"AJA AutoCirculateTransfer failed";
                            return caspar::make_ready_future(false);
                        }

                        ++successful_field_transfers_;

                        if (!auto_circulate_started_ &&
                            successful_field_transfers_ >= 3)
                        {
                            if (!device_.AutoCirculateStart(kChannel))
                            {
                                CASPAR_LOG(error)
                                    << L"Unable to start AJA AutoCirculate field output";
                            ...
                            CASPAR_LOG(info)
                                << L"AJA AutoCirculate field output started after "
                                << successful_field_transfers_
                                << L" buffered fields";
                        }
